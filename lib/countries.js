...
/**
 * Normalize code for lookup (lowercase).
 */
function normCode(v) {
...
/**
 * Sort array by object property (asc or desc).
 */
...
export default class Countries extends EventEmitter {
  constructor(data) {
    super();
  ...
  getCountriesFromLanguage(locale) {
    ...
    for (const row of this.data) {
      ...
      if (Array.isArray(langs) && langs.includes(locale)) list.push(row.code);
    }
    return list;
  }
